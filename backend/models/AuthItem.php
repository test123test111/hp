...
class AuthItem extends BackendActiveRecord{
    ...
    public function rules(){
        return [
            [['name','type'],'required'],
            ['name','unique'],
            [['description','data','rule_name'],'safe'],
        ];
    }
    ...
    public function getAllItems(){
        $provider = new ActiveDataProvider([
            'query' => static::find()
                    ->where(['type'=>1,'description'=>Yii::$app->id]),
            'sort' => [
                'attributes' => ['goods_id'],
            ],
            'pagination' => [
    ...
    public function getPermData(){
        $rs = [];
        $data = static::find()->where(['type'=>2,'description'=>Yii::$app->id])->all();
        if($data){
    ...
    public function getRoles(){
        $rs = [];
        $qs = AuthItem::find()->where(['type'=>1,'description'=>Yii::$app->id])->all();
        if($qs){
            foreach ($qs as $key => $v) {
                $rs[$v['name']] = $v['name'];
            }
    ...
    public function flushPerms(){
        ...
        foreach ($perms as $perm) {
            ...
            $s_perm = $auth->createPermission($perm);
            $s_perm->description = Yii::$app->id;
            $auth->add($s_perm);
        }
        $all_perms = AuthItem::find()->where(['type'=>2,'description'=>Yii::$app->id])->all();
        $rs = [];
    ...
    /**
     * clear user platform permission
     * @param  [type] $platform [description]
     * @return [type]           [description]
     */
    public static function clearUserPermissionByPlatform($uid,$platform){
        $auth_assignments = AssignMent::find()->where(['user_id'=>$uid])->all();
        if(!empty($auth_assignments)){
            foreach($auth_assignments as $assignment){
                $auth_item = static::find()->where(['name'=>$assignment->item_name,'type'=>1,'description'=>$platform])->one();
                if(!empty($auth_item)){
                    AssignMent::deleteAll(['item_name'=>$assignment->item_name,'user_id'=>$uid]);
                }
